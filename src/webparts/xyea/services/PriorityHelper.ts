...
export class PriorityHelper {
  ...
  public static getNextPriority(items: IConvertFileProps[], convertFilesId: number): number {
    const filteredItems = items.filter(item => item.ConvertFilesID === convertFilesId && !item.IsDeleted);
    
    if (filteredItems.length === 0) {
      return 1;
    }

    const maxPriority = Math.max(...filteredItems.map(item => item.Priority));
  ...
  public static normalizePriorities(items: IConvertFileProps[], convertFilesId: number): { 
    ...
    const filteredItems = items
      .filter(item => item.ConvertFilesID === convertFilesId && !item.IsDeleted)
